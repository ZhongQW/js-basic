 ...
 */
#include "Studio.h"
#include "../Persistence/Studio_Persist.c"
#include <stdlib.h>
#include <assert.h>
...
int Studio_Srv_FetchByName(studio_list_t list, char condt[]){
	return Studio_Perst_SelectByName(list,condt);
}

int Studio_Srv_FilterByName(studio_list_t list, char filter[]){
	return Studio_Perst_FilterByName(list,filter);
}
//通过ID在list中查找对应放映厅结点
